...
app.param('spiceIndex', (req, res, next, id) => {
  let spiceID = spiceRack(id);
  try {
    const found = spiceRack.find((spiceIndex) => {
      return spiceIndex === spiceID;
    })
    if (found) {
      req.spell = found;
      next();
    } else {
      next(new Error('Spice not found')):
    };
    } catch (err) {
      next(err)
    }
